...
# Глобальный реестр активных процессов автоответчиков: {account_id: {"client": client, "config_id": id}}
active_autoresponders = {}

def get_db_connection():
    return psycopg2.connect(DATABASE_URL, cursor_factory=RealDictCursor)

# --- Обработчик задач на массовую циклическую / рандомную рассылку ---
            ...
            while sent_count < total_messages:
                for cid in chat_ids:
                    ...
                    try:
                        ...
                        # Шаг обновления прогресс-бара на сайте
...
# --- Динамический хэндлер автоответчика ---
    ...
    async def reply_handler(event):
        ...
        if is_triggered:
            try:
                ...
                print(f"[Autoresponder] Отправлен автоответ в чат {event.chat_id}")
            except Exception as reply_err:
                print(f"Ошибка автоответа: {reply_err}")
                
    return reply_handler

# --- Фоновый воркер опроса состояний СУБД ---
async def database_polling_worker():
    ...
    while True:
        try:
            ...
            # Удаление отключенных слушателей
            for active_id in list(active_autoresponders.keys()):
                if active_id not in db_account_ids:
                    print(f"[Autoresponder] Остановка слушателя для аккаунта ID {active_id}")
                    try:
            ...
            # Добавление новых активных слушателей
            ...
            print(f"[Worker Error] Ошибка цикла синхронизации: {e}")
            
        await asyncio.sleep(5)

async def main():
    asyncio.create_task(database_polling_worker())
    print("[Bot] Диспетчер запущен.")
