...
class VestTrafferWorker:
        ...
        try:
            ...
            for chat_id in chats:
                try:
                    ...
                    print(f"  ✅ {sent}/{len(chats)} -> {chat_id}")
            ...
            await client.disconnect()
            print(f"✅ Рассылка {task_id} завершена ({sent}/{len(chats)})")
    ...
    async def setup_autoresponder(self, responder, account, user_id):
        ...
        try:
            ...
            keywords = responder['keywords'].lower().split(',')
            ...
            async def handler(event):
                try:
                    ...
                    print(f"  🤖 Автоответ -> {event.sender_id}")
...
if __name__ == '__main__':
    try:
        print("🟢 Запуск Vest Traffer System...")
        asyncio.run(main())
