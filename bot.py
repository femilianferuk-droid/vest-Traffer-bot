...
if not BOT_TOKEN or not DATABASE_URL:
    print("❌ Ошибка: BOT_TOKEN и DATABASE_URL должны быть в переменных окружения")
    sys.exit(1)
...
class VestTrafferWorker:
    ...
    async def process_mailing(self, task):
        ...
        print(f"📨 Обработка рассылки #{task_id}")
        
        # Проверка подписки
        if not await self.check_subscription(user_id):
            async with db_pool.acquire() as conn:
                await conn.execute(
                    "UPDATE mailing_task SET status = 'Ошибка: нет подписки' WHERE id = $1",
                    task_id
                )
            print(f"❌ Рассылка #{task_id}: нет подписки")
            return
        ...
        if not account or not account['session_string']:
            async with db_pool.acquire() as conn:
                await conn.execute(
                    "UPDATE mailing_task SET status = 'Ошибка: аккаунт не найден' WHERE id = $1",
                    task_id
                )
            print(f"❌ Рассылка #{task_id}: аккаунт не найден")
            return
        ...
        try:
            ...
            if not await client.is_user_authorized():
                async with db_pool.acquire() as conn:
                    await conn.execute(
                        "UPDATE mailing_task SET status = 'Ошибка: сессия недействительна' WHERE id = $1",
                        task_id
                    )
            ...
            for chat_id in chats:
                try:
                    ...
                    async with db_pool.acquire() as conn:
                        await conn.execute(
                            'UPDATE mailing_task SET sent_count = $1 WHERE id = $2',
                            sent, task_id
                        )
                    
                    print(f"  ✅ [{sent}/{total}] Отправлено в {chat_id}")
            ...
            await client.disconnect()
            print(f"✅ Рассылка #{task_id} завершена ({sent}/{total})")
            
        except Exception as e:
            async with db_pool.acquire() as conn:
                await conn.execute(
                    f"UPDATE mailing_task SET status = 'Ошибка: {str(e)[:50]}' WHERE id = $1",
                    task_id
                )
            print(f"❌ Критическая ошибка в рассылке #{task_id}: {e}")
        ...
        try:
            ...
            async def handler(event):
                try:
                    ...
                    print(f"  🤖 Автоответ для {event.sender_id}")
            ...
            print(f"🤖 Автоответчик #{responder_id} активирован")
            
        except Exception as e:
            print(f"❌ Ошибка настройки автоответчика #{responder_id}: {e}")
    ...
    async def main_loop(self):
        print("🔄 Vest Traffer Worker запущен")
        ...
        while True:
            try:
                ...
                print(f"⚠️ Ошибка в main_loop: {e}")
...
async def cmd_start(message: types.Message):
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Открыть панель", url="https://your-site.vercel.app")]
    ])
    
    await message.answer(
        "Добро пожаловать в Vest Traffer!\n\n"
        "Автоматизация рассылок Telegram\n"
        "Автоответчики на сообщения\n"
        "Управление аккаунтами\n\n"
        "Для доступа к панели управления нажмите кнопку ниже:",
...
if __name__ == '__main__':
    try:
        print("🟢 Vest Traffer System Starting...")
        asyncio.run(main())
